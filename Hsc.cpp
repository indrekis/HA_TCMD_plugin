...
void HAEngine::hsc_cleanup(void) {
...
U16B make_context(unsigned char cl, S16B c);
...
void HAEngine::init_model(void) {
    ...
    elp=(U16B *)malloc(NUMCON*sizeof(*elp));
    eln=(U16B *)malloc(NUMCON*sizeof(*eln));
    cl=(unsigned char *)malloc(NUMCON*sizeof(*cl));
    cc=(unsigned char *)malloc(NUMCON*sizeof(*cc));
    ft=(U16B *)malloc(NUMCON*sizeof(*ft));
    fe=(unsigned char *)malloc(NUMCON*sizeof(*fe));
    rfm=(unsigned char *)malloc(NUMCON*sizeof(*rfm));
    con=(unsigned char (*)[4])malloc(NUMCON*sizeof(*con));
    fc=(unsigned char *)malloc(NUMCFB*sizeof(*fc));
...
void HAEngine::init_pack(void) 
{
    init_model();
    ac_init_encode();
}

void HAEngine::init_unpack(void) 
{
    init_model();
    ac_init_decode();
}
...
void HAEngine::el_movefront(U16B cp) 
{
    if (cp==elf) return;
...
U16B HAEngine::find_longest(void) 
{
    hs[1]=hrt[curcon[0]];	
...
U16B HAEngine::adj_escape_prob(U16B esc, U16B cp) 
{
